"""Análisis de frecuencias de palabras en texto.

Versión optimizada y estructurada siguiendo las mejores prácticas de Python (PEP 8).
"""
...
def obtener_frecuencias(
    ...
    """Calcula la frecuencia de las palabras en un texto y las filtra por umbral.

    Args:
        texto: Texto a analizar.
        min_freq: Frecuencia mínima para incluir la palabra (inclusivo).
        max_freq: Frecuencia máxima para incluir la palabra (inclusivo).
        eliminar_stopwords: Si es True, excluye conectores y preposiciones comunes.

    Returns:
        Diccionario con las palabras filtradas, ordenadas de mayor a menor frecuencia.
    """
    # 1. Extraer palabras en minúsculas (incluyendo tildes y eñes; ignora números y signos)
    palabras = re.findall(r'\b[a-záéíóúüñ]+\b', texto.lower())

    # Palabras vacías habituales en español (útil para análisis semántico)
    if eliminar_stopwords:
        stopwords = {
            'a', 'al', 'como', 'con', 'de', 'del', 'el', 'en', 'es', 'este',
            'la', 'las', 'le', 'lo', 'los', 'más', 'no', 'o', 'para', 'pero',
            'por', 'que', 'se', 'son', 'su', 'sus', 'un', 'una', 'y', 'ya',
        }
        palabras = [p for p in palabras if p not in stopwords and len(p) > 2]

    # 2. Conteo eficiente con collections.Counter (implementado en C)
    conteo = Counter(palabras)

    # 3. Filtrar por rango y devolver ordenado (most_common entrega de mayor a menor)
    return {
        palabra: freq
...
Diferentes instituciones científicas y educativas en todo el mundo se dedican seriamente a la búsqueda de otros planetas (por ejemplo: California & Carnegie Planet Search o The Geneva Extrasolar Planet Search Programmes Archivado el 26 de septiembre de 2006 en Wayback Machine.). Hoy en día se conocen más de 1800 exoplanetas (su número aumenta mes a mes); sin embargo, ninguno de ellos es como la Tierra. Instituciones como el Centro de Astrobiología (CAB) y el Instituto de astrobiología de la NASA (NAI) empiezan a reforzar los pilares para detectar y entender la vida más allá de la Tierra.
...
Es un telescopio espacial en desarrollo por parte de la NASA y Northrop Grumman; su objetivo principal es la detección de planetas comparables a la Tierra mediante el uso de interferometría óptica. El proyecto fue aprobado en 1998 con lanzamiento programado para 2005, pero este fue interrumpido en cinco oportunidades debido a cuestiones presupuestarias.[31][32] Finalmente, la ejecución de la misión fue pospuesta "indefinidamente" en 2008.

Buscador de Planetas Terrestres (Terrestrial Planet Finder)

El Terrestrial Planet Finder (TPF) es un proyecto de la NASA para la creación de un sistema de telescopios capaz de detectar planetas extrasolares similares a la Tierra. Su lanzamiento ha sido pospuesto indefinidamente.[33][34][35][36]
...
La nave espacial Phoenix analizó muestras del suelo y del hielo polar de Marte, en un esfuerzo de identificar zonas habitables en ese planeta.[37]

Astrogenómica

El término Astrogenómica es un concepto basado en la integración de dos ramas importantes de las ciencias biológicas: Genómica y Astrobiología. La genómica, puede contribuir en la investigación de la astrobiología, abordando temas como el origen y la evolución de la vida en la Tierra, como un ejemplo para entender el origen de la vida en cualquier otro lugar del universo[38][39]

Este término fue introducido por primera vez por David S. Holmes,[40] durante la asistencia al Congreso "Integrative Biology Summit-2013" realizado el 5-7 de agosto de 2013 en la ciudad de Las Vegas (EE. UU.), donde fue invitado como uno de los expositores principales.
...
if __name__ == "__main__":
    # Análisis con las mismas condiciones que el script original (frecuencia entre 3 y 100)
    frecuencias_ordenadas = obtener_frecuencias(TEXTO_EJEMPLO, min_freq=3, max_freq=100, eliminar_stopwords=True)

    # Imprimir el diccionario resultante tal como hacía el script original
